...
#include "sycomore/QuantityView.h"
...
{
    auto const & orders = model.orders();
    ...
    {
        auto const order = sycomore::view(orders, i);
        auto const expected_order = sycomore::view(expected_orders, i);
        
        for(std::size_t j=0; j<expected_order.size(); ++j)
        {
            CHECK_QUANTITY(order(j), expected_order(j));
        }
        
        auto const expected_state = xt::view(expected_states, i);
...
BOOST_AUTO_TEST_CASE(Empty, *boost::unit_test::tolerance(1e-9))
{
    ...
    sycomore::TensorQ<2> const orders{{0*rad/m, 0*rad/m, 0*rad/m}};
...
BOOST_AUTO_TEST_CASE(PositiveGradientX, *boost::unit_test::tolerance(1e-9))
{
    ...
    sycomore::TensorQ<2> const orders{
...
BOOST_AUTO_TEST_CASE(TimeIntervalSpeciesOffResonance, *boost::unit_test::tolerance(1e-9))
{
    ...
    sycomore::epg::Discrete3D model({
        species.R1(), species.R2(), species.D(), 10*Hz});
    model.apply_pulse(47*deg, 23*deg);
    model.apply_time_interval(10*ms, {2*mT/m, 0*mT/m, 0*mT/m});
